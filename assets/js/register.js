import {
    ...
    controlEmail, controlTerms, info, successBorder, checkFields, eyePassword, controlPseudo, initialPseudo
} from './dom.js';
window.onload = () => {
    ...
    if (registration_form) {
        ...
        const registration_form_pseudo = registration_form.querySelector('#registration_form_pseudo');
        ...
        registration_form_email.addEventListener('focus', function () {
            ...
            checkFields(registration_form_email, registration_form_pseudo, registration_form_plainPassword, registration_form_agreeTerms, registration_form_submit);
        });
        ...
        registration_form_pseudo.addEventListener('focus', function () {
            information = "Indiquez votre pseudo";
            info(message, information);
            clearField(this);
            clearBorder(this);
            initialPseudo(allowPseudo);
            password_criteria.style.display = "none";
            checkFields(registration_form_email, registration_form_pseudo, registration_form_plainPassword, registration_form_agreeTerms, registration_form_submit);
        });
        ...
        registration_form_plainPassword.addEventListener('blur', function ({ currentTarget }) {
            let password = currentTarget.value;
            if (password.length == 10 && password_length_criteria.classList.contains('password-criteria-true') &&
        ...
        registration_form_submit.addEventListener('click', function (e) {
            ...
            for (let i = 0; i < inputs.length; i++) {
                if (inputs[i].type == "email" || inputs[i].type == "text" || inputs[i].type == "password" || inputs[i].type == 'checkbox') {
            ...
            for (let i = 0; i < champsucess.length; i++) {
                ...
                if (champsucess[i].type == 'text' && champsucess[i].value == "" || champsucess[i].type == 'text' && champsucess[i].classList.contains('border-red-300')) {
                    alertBorder(champsucess[i]);
                    compteur++;
                } else if (champsucess[i].type == 'text' && !champsucess[i].value == "" && champsucess[i].classList.contains('border-green-300')) {
                    successBorder(champsucess[i]);
                    nbBordure++;
                }
                if (champsucess[i].type == 'password' && champsucess[i].value == "" || champsucess[i].type == 'password' && champsucess[i].classList.contains('border-red-300')) {
                    alertBorder(champsucess[i]);
                    compteur++;
                } else if (champsucess[i].type == 'password' && !champsucess[i].value == "" && champsucess[i].classList.contains('border-green-300')) {
                    successBorder(champsucess[i]);
                    nbBordure++;
                }
